          ...
          <table class="w-full">
            ...
            <tbody>
              @if(count($cartItems) > 0)
                  ...
                  <tr wire:key="{{ $item['product_id'] ?? 'item-'.uniqid() }}">
                    <td class="py-4">
                      <div class="flex items-center">
                        ...
                        <span class="font-semibold">{{ $item['name'] ?? 'Unknown Product' }}</span>
                      </div>
                    </td>
                    ...
                    <td class="py-4">
                      <div class="flex items-center">
                        <button wire:click="decrementItem('{{ $item['product_id'] ?? 0 }}')" wire:loading.attr="disabled" class="border rounded-md py-2 px-4 mr-2 hover:bg-gray-100">
                          <span wire:loading.remove wire:target="decrementItem('{{ $item['product_id'] ?? 0 }}')">-</span>
                          <span wire:loading wire:target="decrementItem('{{ $item['product_id'] ?? 0 }}')">...</span>
                        </button>
                        <span class="text-center w-8">{{ $item['quantity'] ?? 1 }}</span>
                        <button wire:click="incrementItem('{{ $item['product_id'] ?? 0 }}')" wire:loading.attr="disabled" class="border rounded-md py-2 px-4 ml-2 hover:bg-gray-100">
                          <span wire:loading.remove wire:target="incrementItem('{{ $item['product_id'] ?? 0 }}')">+</span>
                          <span wire:loading wire:target="incrementItem('{{ $item['product_id'] ?? 0 }}')">...</span>
                        </button>
                      </div>
                    </td>
                    <td class="py-4">Rp{{ number_format($item['price'] * $item['quantity'], 0, ',', '.') }}</td>
                    <td>
                      <button wire:click="removeItem('{{ $item['product_id'] ?? 0 }}')" wire:loading.attr="disabled" class="bg-slate-300 border-2 border-slate-400 rounded-lg px-3 py-1 hover:bg-red-500 hover:text-white hover:border-red-700">
                        <span wire:loading.remove wire:target="removeItem('{{ $item['product_id'] ?? 0 }}')">Remove</span>
                        <span wire:loading wire:target="removeItem('{{ $item['product_id'] ?? 0 }}')">Removing...</span>
                      </button>
                    </td>
                  </tr>
                  @endforeach
